...
function LogIn(props) {
  return (
    <button type="button" onClick={props.isClickFun}>
      {props.name}
    </button>
  );
}
...
function Example(props) {
  if (!props.toDisplay) {
    ...
    return <h5>Component Rendered</h5>;
  }
}
  ...
  render() {
    ...
    return (
      <div>
        ...
        {loggedIn ? (
          <LogIn isClickFun={this.IfLoggedIn} name="LogIn" />
        ) : (
          <LogOut isClickFun={this.IfLoggedOut} name="LogOut" />
        )}
